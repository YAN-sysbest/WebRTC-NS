...
int WebRtcNs_InitCore(NoiseSuppressionC *self, uint32_t fs) {
    ...
    for (i = 0; i < SIMULT * HALF_ANAL_BLOCKL; i++) {
        self->lquantile[i] = 8.f;//对数分位数
        self->density[i] = 0.3f;//概率密度
    }
    ...
    for (i = 0; i < HALF_ANAL_BLOCKL; i++) {
        // Smooth LR (same as threshold).平滑处理过的噪声判决似然比门限
        self->logLrtTimeAvg[i] = LRT_FEATURE_THR;
    }

    // Feature quantities.
    // Spectral flatness (start on threshold).频谱平坦度
    ...
    // Average LRT factor (start on threshold).平均LRT因子
    self->featureData[3] = LRT_FEATURE_THR;
    // Spectral template diff (start on threshold).频谱模板差异度
...
static void ComputeSpectralFlatness(NoiseSuppressionC *self,
    ...
    avgSpectralFlatnessDen = self->sumMagn;//含直流Yk总和
    for (i = 0; i < shiftLP; i++) {
        avgSpectralFlatnessDen -= magnIn[i];//去直流Yk总和
    }
    // Compute log of ratio of the geometric to arithmetic mean: check for log(0)
    // case.
    for (i = shiftLP; i < self->magnLen; i++) {
        if (magnIn[i] > 0.0) {
            avgSpectralFlatnessNum += logf(magnIn[i]);//取log便于计算乘积
        } else {
    ...
    self->featureData[0] += SPECT_FL_TAVG * (spectralTmp - self->featureData[0]);//平坦度平滑处理
    // Done with flatness feature.
}
...
static void ComputeSpectralDifference(NoiseSuppressionC *self,
    ...
    avgPause /= self->magnLen;//求模板均值
    avgMagn /= self->magnLen;//求输入均值
    ...
    for (i = 0; i < self->magnLen; i++) {
        covMagnPause += (magnIn[i] - avgMagn) * (self->magnAvgPause[i] - avgPause);//求输入和模板协方差
        varPause +=
                (self->magnAvgPause[i] - avgPause) * (self->magnAvgPause[i] - avgPause);//求模板方差
        varMagn += (magnIn[i] - avgMagn) * (magnIn[i] - avgMagn);//求输入方差
    }
    ...
    avgDiffNormMagn =
            varMagn - (covMagnPause * covMagnPause) / (varPause + 0.0001f);//频谱模板差异特征计算公式
    // Normalize and compute time-avg update of difference feature.
    avgDiffNormMagn = avgDiffNormMagn / (self->featureData[5] + 0.0001f);
    self->featureData[4] +=
            SPECT_DIFF_TAVG * (avgDiffNormMagn - self->featureData[4]);//平滑更新
}
...
static void SpeechNoiseProb(NoiseSuppressionC *self,
    ...
    for (i = 0; i < self->magnLen; i++) {
        ...
        self->logLrtTimeAvg[i] +=
                LRT_TAVG * (besselTmp - logf(tmpFloat1) - self->logLrtTimeAvg[i]);//LRT计算，与理论些许不同
    ...
    if (logLrtTimeAvgKsum < threshPrior0) {
        widthPrior = widthPrior1;//LRT特征过小，取w大一些即2倍
    }
    // Compute indicator function: sigmoid map.平滑处理 sigmoid映射
    indicator0 =
            0.5f *
            (tanhf(widthPrior * (logLrtTimeAvgKsum - threshPrior0)) + 1.f);//映射函数M(z) = 0.5*(tanh(w1z1) + 0.5) ,z1 = 特征 - 阈值参数 
     
    // Spectral flatness feature.
    tmpFloat1 = self->featureData[0];//data0为频谱平坦度
    widthPrior = widthPrior0;
    // Use larger width in tanh map for pause regions.
    ...
    indicator1 =
            ...
            (tanhf((float) sgnMap * widthPrior * (threshPrior1 - tmpFloat1)) +
             1.f);//映射函数M(z) = 0.5*(tanh(w2z2) + 0.5) ,z1 = 特征 - 阈值参数 

    // For template spectrum-difference.
    tmpFloat1 = self->featureData[4]; //data4为频谱模板差异特征
    widthPrior = widthPrior0;
    // Use larger width in tanh map for pause regions.
    if (tmpFloat1 < threshPrior2) {
        widthPrior = widthPrior2; //频谱模板差异特征过小，取w大一些即2倍
    }
    // Compute indicator function: sigmoid map.
    indicator2 =
            0.5f * (tanhf(widthPrior * (tmpFloat1 - threshPrior2)) + 1.f);//映射函数M(z) = 0.5*(tanh(w1z1) + 0.5) ,z1 = 特征 - 阈值参数

    // Combine the indicator function with the feature weights.
    indPrior = weightIndPrior0 * indicator0 + weightIndPrior1 * indicator1 +
               weightIndPrior2 * indicator2;//综合所有特征，加权求和
    // Done with computing indicator function.

    // Compute the prior probability.
    self->priorSpeechProb += PRIOR_UPDATE * (indPrior - self->priorSpeechProb);//平滑处理
...
static void UpdateNoiseEstimate(NoiseSuppressionC *self,
    ...
    for (i = 0; i < self->magnLen; i++) {
        ...
        if (probSpeech > PROB_RANGE) {
            gammaNoiseTmp = SPEECH_UPDATE;//语音概率高，增加平滑参数，防止语音开始处噪声水平过高
        }
        ...
        if (gammaNoiseTmp == gammaNoiseOld) {
            ...
            if (noiseUpdateTmp < noise[i]) {
                noise[i] = noiseUpdateTmp;//前一帧噪声估计小，选用前一帧
            }
        }
    }  // End of freq loop.
}
...
void WebRtcNs_AnalyzeCore(NoiseSuppressionC *self, const int16_t *speechFrame) {
    ...
    float real[ANAL_BLOCKL_MAX] = { 0 }, imag[HALF_ANAL_BLOCKL] = { 0 };
    ...
    UpdateBuffer(speechFrame, self->blockLen, self->anaLen, self->analyzeBuf);//进160点，取前一帧后64点和本帧160，得256点
    //for (int aa = 0; aa < 160; aa++) printf("%d   %d   %d\n", aa,speechFrame[aa], self->analyzeBuf[aa]);

    Windowing(self->window, self->analyzeBuf, self->anaLen, winData);//加窗 windata输出
    ...
    self->blockInd++;  // Update the block index only when we process a block.第几帧
    ...
    for (i = 0; i < self->magnLen; i++) {
        ...
        sumMagn += magn[i];
        if (self->blockInd < END_STARTUP_SHORT) {//前50帧
            if (i >= kStartBand) {
...
void WebRtcNs_ProcessCore(NoiseSuppressionC *self,
    ...
    if (self->blockInd < END_STARTUP_SHORT) {//初始幅度谱估计
        for (i = 0; i < self->magnLen; i++) {
            self->initMagnEst[i] += magn[i];
        }
    }

    ComputeDdBasedWienerFilter(self, magn, theFilter);//计算维纳滤波器的频率响应

    for (i = 0; i < self->magnLen; i++) {//维纳滤波器限幅
        // Flooring bottom.
        ...
        real[i] *= self->smooth[i];//频域相乘，实际矩阵相乘
        imag[i] *= self->smooth[i];//频域相乘，实际矩阵相乘
    }
    ...
    if (self->gainmap == 1 && self->blockInd > END_STARTUP_LONG) {
        ...
        gain = sqrtf(energy2 / (energy1 + 1.f));//缩放因子K，帮助重建语音帧
