...
int main() {
    ...
    fp_in = fopen("sp0102noise.pcm", "rb");//123 music speech sp0102noise
    if (fp_in == NULL) { printf("Cann't open the read file!!!\n");  exit(0); }
    memset(buffer1, 0, sizeof(buffer1));
    //fread(&buffer1, 2, 22, fp_in);
    fp_out = fopen("out.pcm", "wb");
    if (fp_out == NULL) { printf("Cann't open the out file!!!\n");  exit(0); }

    status = WebRtcNs_set_policy(nsHandle, 3);//kModerate

    while (!feof(fp_in)) {
        fread(buffer1, 2, frames, fp_in);
        noise_suppression(buffer1, sampleRate, inSampleCount, nsHandle, status);
        fwrite(buffer1, 2, frames, fp_out);
    }
